...
```sh
python3 homun/fixtures/shared-world-app/seed.py          # seed starter tasks
python3 homun/fixtures/shared-world-app/checkpoint.py    # -> count=3 hash=...
HOST=0.0.0.0 PORT=3000 python3 homun/fixtures/shared-world-app/server.py
# open http://127.0.0.1:3000/, add a task, then re-run checkpoint.py -> count grows
```

## How the lab uses it

`homun/labs/shared-world-concurrent-live.yaml` clones this repo into the subject sandbox,
...
`HOMUN_LIVE_SHARED_WORLD=1`) drives 3 concurrent personas against it.

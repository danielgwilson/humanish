# Homun Dogfood Config

This directory is the committed source plane for dogfooding `homun` with
Homun itself.
...
Executable browser journeys live in `homun/scenarios/*.yaml` under
`browser.steps`. `homun run --app-url <loopback-url>` uses those steps when

...
Analysis is opt-in. Inspect the local admission estimate, then generate:

```bash
npx humanish analyze --run latest --max-cost 2 --dry-run --json
npx humanish analyze --run latest --max-cost 2 --json

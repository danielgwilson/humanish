...
Lane and roster-group objects reject unknown fields before a run starts. For
example, `runtme` produces `HUMANISH_LAB_INVALID` with the group or lane index;
it cannot silently become a default lane. Use `instruction` for a lane's steer.

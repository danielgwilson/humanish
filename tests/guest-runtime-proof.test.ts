...
import { GUEST_PROOF_CASES, parseGuestProofResult, parseGuestRelayImport, validateGuestProofCell } from '../scripts/guest-runtime-proof-contract.mjs';
function sample(mode='packaged-main') {
 return {mode,commandFailure:undefined as string|undefined,relayModuleSha256:'a'.repeat(64),relayImport:{passed:true,afVsock:true,pythonVersion:'3.13.5',moduleSha256:'a'.repeat(64),uid:1000,gid:1000},state:{Status:'exited',ExitCode:0,OOMKilled:false},hostConfig:{Memory:1610612736,MemorySwap:1610612736,ShmSize:268435456,ReadonlyRootfs:true,NetworkMode:'none',Privileged:false,PidsLimit:256,Binds:[],Devices:[],DeviceRequests:[]},mounts:[],
...
describe('constrained guest proof receipts',()=>{
 ...
 it.each([
  ...
  ,(s:ReturnType<typeof sample>)=>{s.relayImport.passed=false;}
  ,(s:ReturnType<typeof sample>)=>{s.relayImport.afVsock=false;}
  ,(s:ReturnType<typeof sample>)=>{s.relayImport.pythonVersion='3.12.3';}
  ,(s:ReturnType<typeof sample>)=>{s.relayImport.moduleSha256='b'.repeat(64);}
  ,(s:ReturnType<typeof sample>)=>{s.relayImport.uid=0;}
 ...
 it('requires one actual relay-import marker',()=>{
  expect(parseGuestRelayImport('HUMANISH_RELAY_IMPORT {"passed":true}\n')).toEqual({passed:true});
  expect(()=>parseGuestRelayImport('')).toThrow();
  expect(()=>parseGuestRelayImport('HUMANISH_RELAY_IMPORT {}\nHUMANISH_RELAY_IMPORT {}\n')).toThrow();
 });
});

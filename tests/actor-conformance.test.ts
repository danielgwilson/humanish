...
describe("scripted-browser ActorTrace conformance", () => {
  it("a scripted session trace conforms to mimetic.actor-trace.v1", async () => {
    const artifactRoot = await mkdtemp(path.join(tmpdir(), "mimetic-scripted-conformance-"));
    ...
    try {
      const result = await runScriptedBrowserSession({
        ...
        journey: {
          ...
          source: "mimetic/scenarios/conformance-journey.yaml",

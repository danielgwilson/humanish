...
function restoreBrowserEnv(previousBrowserCommand: string | undefined, previousBrowserPersonaDriver: string | undefined): void {
  if (previousBrowserCommand === undefined) {
    ...
    process.env.HUMANISH_BROWSER_COMMAND = previousBrowserCommand;
  }
  if (previousBrowserPersonaDriver === undefined) {
    delete process.env.HUMANISH_BROWSER_PERSONA_DRIVER;
  } else {
    process.env.HUMANISH_BROWSER_PERSONA_DRIVER = previousBrowserPersonaDriver;
  }
}

async function writeHumanishBrowserScenario(cwd: string, scenarioText: string): Promise<void> {
...
describe("dry-run bundles", () => {
  ...
  it("captures and verifies live browser app desktop/mobile evidence", async () => {
    await withFixtureCopy(async (cwd) => {
      await withHttpServer(async (appUrl) => {
        ...
        const previousBrowserPersonaDriver = process.env.HUMANISH_BROWSER_PERSONA_DRIVER;
        process.env.HUMANISH_BROWSER_COMMAND = await writeFakeBrowserCommand(cwd);
  ...
  it("keeps a blocked browser run structurally verifiable without claiming screenshots it never wrote", async () => {
    ...
    await withFixtureCopy(async (cwd) => {
      await withHttpServer(async (appUrl) => {
        ...
        process.env.HUMANISH_BROWSER_COMMAND = await writeNeverScreenshottingBrowserCommand(cwd);
        process.env.HUMANISH_BROWSER_PERSONA_DRIVER = "fixture";
  ...
  it("uses executable browser scenario manifests for browser app proof traces", async () => {
    await withFixtureCopy(async (cwd) => {
      await withHttpServer(async (appUrl) => {
        await writeHumanishBrowserScenario(
          cwd,
          [
  ...
  it("rejects non-loopback app URLs for browser app proof", async () => {
    await withFixtureCopy(async (cwd) => {
      ...
      expect(result.ok).toBe(false);
      expect(result.error?.code).toBe("HUMANISH_INVALID_APP_URL");
    });
  });
  ...
  it("exposes explicit local Codex TUI actor runs through the Commander CLI", async () => {
    await withFixtureCopy(async (cwd) => {
      ...
      try {
        ...
        expect(observer.ok).toBe(true);
        expect(observer.observerPath).toBe(".humanish/runs/codex-cli/observer/index.html");
      } finally {
        if (previousCommand === undefined) {
          delete process.env.HUMANISH_CODEX_ACTOR_COMMAND;
        } else {
          process.env.HUMANISH_CODEX_ACTOR_COMMAND = previousCommand;
        }
      }
    });
  });
  ...
  it("publishes running Observer data while local Codex exec actors are active", async () => {
    await withFixtureCopy(async (cwd) => {
      ...
      const runRoot = path.join(cwd, ".humanish/runs/codex-exec-live-follow-test");
      await writeFile(
        fakeActor,
        [
          "process.stdout.write('{\"type\":\"turn.started\"}\\n');",
  ...
  it("exposes explicit local Codex exec actor runs through the Commander CLI", async () => {
    await withFixtureCopy(async (cwd) => {
      ...
      try {
        ...
        const observer = JSON.parse(watch.stdout) as { ok: boolean; observerPath: string };
        expect(observer.ok).toBe(true);
        expect(observer.observerPath).toBe(".humanish/runs/codex-exec-cli/observer/index.html");
      } finally {
        if (previousCommand === undefined) {
          delete process.env.HUMANISH_CODEX_ACTOR_COMMAND;
        } else {
  ...
  it("runs an explicit Codex app-server actor with redacted protocol evidence", async () => {
    await withFixtureCopy(async (cwd) => {
      ...
      expect(bundle.streams[0]?.codex.trace.schema).toBe("humanish.codex-app-server-trace.v1");
      ...
      expect(appServerEvents).not.toContain("You are a Humanish Codex app-server dogfood actor");
    ...
    await withFixtureCopy(async (cwd) => {
      ...
      try {
        ...
        if (previousActorCommand === undefined) {
          ...
          process.env.HUMANISH_CODEX_ACTOR_COMMAND = previousActorCommand;
        }
      }
    });
  });

---
name: homun
description: Install and configure Homun CLI in a JavaScript app as an open-source-safe persona simulation harness. Use when an agent needs to add homun, run safe first setup, create synthetic personas or scenarios, configure env var names without values, run verification and Observer commands, or draft public-safe feedback issues without GitHub mutation.
---

# Homun CLI

Use this skill to add Homun to a target app without relying on chat memory or
...
2. Install Homun with the repo's package manager:

   ```bash
   npm i -D homun
   ```

   The package is `homun`; the installed binary is `homun`. After
   installation, `npx homun ...` resolves the local project binary. For a
   one-shot command before installation, use
   `npx --package homun homun ...` so npm does not resolve an unrelated
   package named `homun`.

3. Preview setup:

   ```bash
   npx homun init --dry-run --json
   ```

4. Apply setup after the planned changes are understood:

   ```bash
   npx homun init --yes --json
   ```

5. Confirm the layout:
   - commit `homun/` source files;
   - ignore `.homun/` runtime artifacts;
   - keep committed labs under `homun/labs/*.yaml`;
...
When creating or editing Homun files:

- use `.yaml` for human-authored Homun source: personas, scenarios,
  policies, labs, review vocabulary, and milestones;
- use `.ts` for executable integration: `homun/config.ts`, adapters, route
...
Do not create `.yml` files under `homun/`; `.yml` is for outside ecosystem
...
Create or edit only synthetic files under `homun/`.
...
repo's `homun/` files, not in the package or this skill.

When the app can run locally, make at least one scenario executable with a
`browser.steps` manifest so `homun run --app-url` can drive the app instead
...
`npx homun lab inspect <lab>` to see how a manifest parses, including
warnings for fields the engine does not consume yet.

Use committed `homun/labs/*.yaml` for public-safe, reproducible labs. Use
ignored `.homun/labs/*.yaml` or `.homun/local/labs/*.yaml` for private repo
...
npx homun lab run first-run --json --no-open
```

## First Proof Run

Run the no-credentials path first. This proves Homun artifact plumbing, not
target app behavior:
...
npx homun run --app-url http://127.0.0.1:<port> --sims 2 --json
npx homun verify --run latest --json
npx homun watch --run latest --detach --no-open --json
```

Do not use `homun watch --sims ...` as a substitute for app-url proof.
...
```bash
npx homun watch .homun/labs/local-live.yaml --env-file .homun/local/provider.env
```
...
- generated local artifact paths under `.homun/`;

...
Package/source version in this tree: `0.102.0` (2026-09-25); latest published is `0.101.0`. The Observer is phone-usable as a stated requirement (observer/AGENTS.md); interactive primitives start from Base UI. The Observer renderer is the observer/ workspace artifact only; the legacy string-concat renderer was deleted at cutover (#426), and rollback is a version pin to `0.42.0`. The containment boundary introduced in
...
optional native camera and speech. Linux live acceptance and the updated Mac
conversation passed, including spoken clarification of a misheard answer. The
Mac recording's automatic analysis failed validation; resolving that report
failure remains required before merge and publication.

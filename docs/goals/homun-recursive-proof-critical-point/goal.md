# Goal: Homun Recursive Proof Critical Point

Status date: 2026-06-04

## Objective

Prove one bounded public-safe recursive Homun lane where Homun observes a
headed E2B desktop in which a Codex-like actor sets up Homun in a disposable
public OSS app and produces nested Homun browser evidence against the running
app.
...
- `homun run --app-url <loopback-url>` captures live desktop and mobile
  browser evidence against a running local app URL.
- `homun verify` fails if a run claims screenshot or trace evidence that is
  missing on disk.
- `homun lab oss` uses the nested app URL proof when a target app is running,
...
- Provider secrets are not exposed to repo-controlled install, app, or nested
  Homun commands; Codex auth is scoped to the single actor invocation and
  GitHub auth is scoped to clone/askpass only.
- Top-level Observer evidence shows the E2B desktop lane.
- Nested Homun Observer evidence exists inside the desktop lane and contains
...
- Coding-agent actor evidence shows the actor materially discovered, installed,
  initialized, configured, or ran Homun for the target app. Deterministic
...
- Read:
  - the full repo
  - ignored `.homun/` runtime artifacts for verification only
- Commands:
  ...
  - `pnpm check`
- `pnpm release:check`
- bounded `pnpm homun -- lab oss ...` live runs within the spend policy;
...
- Do not commit generated `.homun/` runtime artifacts.
...
- A live bounded run proves headed E2B stream launch, target app setup, nested
  Homun run, nested Observer presence, screenshot evidence, and top-level
  Observer rendering.
...
pnpm homun -- lab oss --detach --open --count 1 --repos maciekt07/TodoApp
...
- `docs/goals/homun-recursive-proof-critical-point/state.yaml`
- `docs/goals/homun-recursive-proof-critical-point/receipts/*.md`
- ignored `.homun/runs/<run-id>/run.json`
- ignored `.homun/runs/<run-id>/review.md`
- ignored `.homun/runs/<run-id>/observer/observer-data.json`
- ignored `.homun/runs/<run-id>/screenshots/*.png`
...
- Provider timeout: every launched E2B desktop must have a bounded
  `HOMUN_E2B_TIMEOUT_MS` and `lifecycle.onTimeout=kill`.
...
- Usage receipt path: `docs/goals/homun-recursive-proof-critical-point/receipts/`.
- Provider cleanup/readback: after live proof, query running Homun OSS
...
- Secret scoping: never forward `OPENAI_API_KEY`, `CODEX_API_KEY`,
  ...
  repo-controlled remote commands. Use Homun-private handoff vars, unset them

...
pnpm homun -- lab oss --detach --open --count 1 --repos reduxjs/redux-essentials-example-app
```
...
- Durable artifacts persisted public repo slug, public-safe log tail, local loopback URLs, synthetic Homun starter output, and screenshot evidence.
...
nested homun: live app-url proof, sims: 2
...
```text
.homun/runs/oss-meta-2026-06-04T08-33-55-996Z-8854989a/run.json
.homun/runs/oss-meta-2026-06-04T08-33-55-996Z-8854989a/review.md
.homun/runs/oss-meta-2026-06-04T08-33-55-996Z-8854989a/observer/index.html
.homun/runs/oss-meta-2026-06-04T08-33-55-996Z-8854989a/observer/observer-data.json
.homun/runs/oss-meta-2026-06-04T08-33-55-996Z-8854989a/screenshots/oss-01-desktop.png
```
...
- the nested Homun Observer open with desktop and mobile browser-surface lanes;
...
- `antfu-collective/vitesse-lite`: failed because the old bootstrap installed Homun with npm against a pnpm repo using `catalog:` dependencies.
...
- Provider cleanup: seven running Homun OSS meta-lab sandboxes were found by
...
`tool=homun&mode=oss-meta-lab`. That provider readback found seven running
Homun OSS meta-lab sandboxes. The old total run-count cap was an arbitrary
...
readback returned zero running Homun OSS meta-lab sandboxes. Exact provider
...
running Homun OSS meta-lab desktops, and cleanup/readback to zero after proof
collection.

Follow-up code guard: live OSS meta-lab launches now check existing running E2B
sandboxes with Homun OSS meta-lab metadata and fail closed when running plus
requested headed desktops would exceed the configured cap.

## Remaining Limits

- This is a phase-change proof, not platform readiness.
- The nested Homun proof captures desktop/mobile render evidence and HTTP

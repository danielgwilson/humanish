...
`CuaExecutor` — keeping mimetic's composition (personas, Observer, `ActorTrace`
...
- **`buildProvider` is REQUIRED alongside `buildExecutor`** — boot-time
  `MIMETIC_CUA_LAB_EXECUTOR_NO_PROVIDER`, emitted in the early fail-closed block
...
- **B — `subject.source: local-app`, fail-closed/library-assisted.** A real,
  ...
  cua backend and returns `MIMETIC_CUA_LAB_LOCAL_APP_NO_EXECUTOR` when no

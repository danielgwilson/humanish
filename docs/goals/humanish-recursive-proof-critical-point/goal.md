# Goal: Humanish Recursive Proof Critical Point

Status date: 2026-06-04

## Objective

Prove one bounded public-safe recursive Humanish lane where Humanish observes a
headed E2B desktop in which a Codex-like actor sets up Humanish in a disposable
public OSS app and produces nested Humanish browser evidence against the running
app.
...
- `humanish run --app-url <loopback-url>` captures live desktop and mobile
  browser evidence against a running local app URL.
- `humanish verify` fails if a run claims screenshot or trace evidence that is
  missing on disk.
- `humanish lab oss` uses the nested app URL proof when a target app is running,
...
- Provider secrets are not exposed to repo-controlled install, app, or nested
  Humanish commands; Codex auth is scoped to the single actor invocation and
  GitHub auth is scoped to clone/askpass only.
- Top-level Observer evidence shows the E2B desktop lane.
- Nested Humanish Observer evidence exists inside the desktop lane and contains
...
- Coding-agent actor evidence shows the actor materially discovered, installed,
  initialized, configured, or ran Humanish for the target app. Deterministic
...
- Read:
  - the full repo
  - ignored `.humanish/` runtime artifacts for verification only
- Commands:
  ...
  - `pnpm check`
- `pnpm release:check`
- bounded `pnpm humanish -- lab oss ...` live runs within the spend policy;
...
- Do not commit generated `.humanish/` runtime artifacts.
...
- A live bounded run proves headed E2B stream launch, target app setup, nested
  Humanish run, nested Observer presence, screenshot evidence, and top-level
  Observer rendering.
...
HUMANISH_E2B_REQUEST_TIMEOUT_MS=30000 \
HUMANISH_E2B_TIMEOUT_MS=3600000 \
HUMANISH_OSS_META_COMPLETION_TIMEOUT_MS=900000 \
HUMANISH_OSS_META_COMPLETION_INTERVAL_MS=10000 \
HUMANISH_OSS_META_ACTOR_FIRST=1 \
HUMANISH_OSS_META_REQUIRE_ACTOR=1 \
HUMANISH_OSS_META_ACTOR_TIMEOUT_MS=480000 \
pnpm humanish -- lab oss --detach --open --count 1 --repos maciekt07/TodoApp
pnpm humanish -- verify --run oss-meta-app-target-main-2026-06-04T19-18Z --json
```
...
- `docs/goals/humanish-recursive-proof-critical-point/state.yaml`
- `docs/goals/humanish-recursive-proof-critical-point/receipts/*.md`
- ignored `.humanish/runs/<run-id>/run.json`
- ignored `.humanish/runs/<run-id>/review.md`
- ignored `.humanish/runs/<run-id>/observer/observer-data.json`
- ignored `.humanish/runs/<run-id>/screenshots/*.png`
...
- Provider timeout: every launched E2B desktop must have a bounded
  `HUMANISH_E2B_TIMEOUT_MS` and `lifecycle.onTimeout=kill`.
...
- Usage receipt path: `docs/goals/humanish-recursive-proof-critical-point/receipts/`.
- Provider cleanup/readback: after live proof, query running Humanish OSS
...
- Secret scoping: never forward `OPENAI_API_KEY`, `CODEX_API_KEY`,
  ...
  repo-controlled remote commands. Use Humanish-private handoff vars, unset them

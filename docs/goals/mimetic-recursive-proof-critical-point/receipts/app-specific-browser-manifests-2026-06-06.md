...
This receipt covers the slice where Mimetic scenario source can define
...
artifacts remain under ignored `.mimetic/` paths.

## Implementation Delta

- `mimetic/scenarios/*.yaml` can now provide executable browser steps through
  a scenario-level `browser.steps` block or compatible step-level
  `steps[].browser` shape.
- `mimetic run --app-url` selects the first executable browser scenario from
...
- The OSS meta-lab bootstrap writes an app-surface browser scenario after the
  target app is HTTP-ready, then runs nested Mimetic live app-url proof.
...
```bash
MIMETIC_BROWSER_COMMAND="<local browser executable>" \
pnpm mimetic -- run \
...
- scenario source: `mimetic/scenarios/fixture-onboarding.yaml`;
...
- scenario source: `mimetic/scenarios/app-surface-browser.yaml`;
- counts: `4/4` steps passed across `2` browser surfaces;
- top-level Observer emitted `oss-meta.nested.step_trace.summary`;
- `pnpm mimetic -- verify --run oss-meta-2026-06-06T04-32-27-418Z-f8642378 --json`
...
- counts: `4/4` steps passed across `2` browser surfaces;
- top-level Observer emitted `oss-meta.nested.step_trace.summary`;
- `pnpm mimetic -- verify --run oss-meta-2026-06-06T04-36-08-860Z-11dd0611 --json`
  passed schema, redaction, review artifact, public-safety, local evidence, and
  Codex app-server evidence checks.
...
Mimetic watcher process after the run.
...
summary: Generated Mimetic setup for lissy93/dashy was ceremonial
...
- schema: `mimetic.feedback-result.v1`;
- `ok: true`;
- source candidate id: `study-quality-oss-01-desktop`;
- GitHub mutation: not performed;
- evidence paths point only to local ignored `.mimetic/` artifacts.

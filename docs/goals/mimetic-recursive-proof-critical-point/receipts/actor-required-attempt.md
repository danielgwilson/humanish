...
goal: a coding-agent persona must materially drive Mimetic setup/use, and the
top-level run must not pass unless that actor evidence reaches a terminal
successful state.

## Code Fixes Before Attempt

- Added `MIMETIC_OSS_META_ACTOR_FIRST=1` to start the Codex actor before
  deterministic setup/readback.
- Added `MIMETIC_OSS_META_REQUIRE_ACTOR=1` and
  `MIMETIC_OSS_META_ACTOR_TIMEOUT_MS` to require terminal actor readback.
...
```text
.mimetic/runs/oss-meta-2026-06-04T09-14-56-290Z-3f979fc9/run.json
.mimetic/runs/oss-meta-2026-06-04T09-14-56-290Z-3f979fc9/review.md
.mimetic/runs/oss-meta-2026-06-04T09-14-56-290Z-3f979fc9/observer/index.html
.mimetic/runs/oss-meta-2026-06-04T09-14-56-290Z-3f979fc9/observer/observer-data.json
.mimetic/runs/oss-meta-2026-06-04T09-14-56-290Z-3f979fc9/screenshots/oss-01-desktop.png
```

Visual inspection:

- Redux Essentials app was visible in desktop and compact browser windows.
- Nested Mimetic Observer was visible and showed desktop/mobile browser lanes.
...
- app surface, nested live Mimetic proof, nested verify, nested Observer, and
...
- remote actor command now maps Mimetic-private Codex auth to `CODEX_API_KEY`
...
```text
exit_status: 0
last_message: MIMETIC_USER_AUTH_OK
```
...
- OpenAI/Codex actor auth is forwarded under Mimetic-private env names, moved
  into shell-local variables, then removed from exported env before repo clone,
  dependency install, app startup, and nested Mimetic commands.
- Codex auth is injected only into the single `codex exec` actor process as
  `CODEX_API_KEY` and/or `CODEX_ACCESS_TOKEN`.
- GitHub token, when present, is forwarded under a Mimetic-private env name and
...
- raw repo clone, `.git` metadata, raw Codex last-message output, and schema
  scratch files were moved to `.mimetic/tmp` and deleted;
...
- remote actor command now passes
  `-m ${MIMETIC_OSS_META_ACTOR_MODEL:-gpt-5.4-mini}`;
- `MIMETIC_OSS_META_ACTOR_MODEL` is forwarded as a public-safe control flag.
...
verify: passed
provider readback: 0 running Mimetic OSS meta-lab sandboxes
```

...
remaining hard requirement is coding-agent-authored Mimetic setup/use evidence;
...
pnpm mimetic -- lab oss --json --no-open --detach --no-redact-repos \
...
- `pnpm mimetic -- verify --run host-plan-preflight-10-2026-06-04 --json`
  passed;
...
- Host actor public repo clones now happen under `.mimetic/tmp/host-actors/...`
...
- `mimetic verify` now walks run text artifacts and public-proof paths, not only
...
- Stale runtime-only profile/raw-clone artifacts were removed from `.mimetic/runs`
...
- remote actor command now passes `-m ${MIMETIC_OSS_META_ACTOR_MODEL:-gpt-5.4-mini}`;
...
nested Mimetic runs live desktop/mobile browser proof, nested verify passes,
...
install, initialize, configure, or run Mimetic.
...
- durable public-safe actor evidence showing material Mimetic setup/use;

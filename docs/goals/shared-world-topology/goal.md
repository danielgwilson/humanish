...
The workshop concluded concurrent multi-seat is "impossible" — but it only inspected mimetic's
...
- **`RunBundle.sharedWorld?` (schema `mimetic.shared-world.v1`)** — additive block: `topology`,
...
- **Evidence (`src/run.ts`):** `RunBundle.attributionClass` + `RunBundle.sharedWorld`
  (`mimetic.shared-world.v1`). `validateSharedWorldEvidence` wired into `verifyRun` (mirror

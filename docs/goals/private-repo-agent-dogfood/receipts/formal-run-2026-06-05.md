...
dogfood run for Homun. Repo labels are redacted as `repo-01` through
`repo-04`.
...
- mode: `live`
- observer: `.homun/runs/oss-meta-2026-06-05T07-02-38-522Z-ea401be2/observer/index.html`
- bundle: `.homun/runs/oss-meta-2026-06-05T07-02-38-522Z-ea401be2/run.json`
- verification: `pnpm homun -- verify --run oss-meta-2026-06-05T07-02-38-522Z-ea401be2 --json` passed all checks.

Verification checks that passed:

- `run.json` exists;
- run schema is `homun.run-bundle.v1`;
...
| `repo-03` | passed | 5/5 checks passed | running app, nested verify, nested Observer, visible layout | present | useful | actionable Homun-owned proof-path confusion |
| `repo-04` | passed | 5/5 checks passed | running app, nested verify, nested Observer, visible layout | present | ceremonial | actionable Homun-owned proof-path confusion |

Setup-quality checks per lane:

- `homun/config.ts` exists;
- 2 persona files detected;
- 2 scenario files detected;
- `package.json` exposes a Homun script;
- `.gitignore` ignores `.homun/`.
...
- All four lanes successfully installed/configured Homun enough to produce
...
## Homun-Owned Follow-Up

Implemented locally in the current branch:

- fixed the pnpm workspace-root Homun install path by using an explicit
  workspace-root dev dependency install;
- replaced a private repo slug in tests with synthetic fixture labels;
- strengthened the Homun skill, agent resource prompt, README command table,
...
- Provider readback before cleanup found `4` running Homun OSS meta-lab
  sandboxes.
- Cleanup killed `4/4`.
- Cleanup errors: `0`.
- Provider readback after cleanup found `0` running Homun OSS meta-lab
  sandboxes.
...
and an invariant-by-invariant completion audit after the Homun-owned local

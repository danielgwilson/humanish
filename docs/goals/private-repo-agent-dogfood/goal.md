...
Daniel-authorized private repo tries to install, configure, and use Humanish;
then evaluate the resulting filesystem state, generated Humanish artifacts,
...
- `docs/goals/humanish-recursive-proof-critical-point/goal.md`.
...
current `.humanish/` evidence and a fresh completion audit.
...
- Each lane records whether the target app reached a local running URL and
  whether nested Humanish proof reached `run --app-url` style browser evidence.
- Each lane records setup-quality filesystem evidence: `humanish/config.ts`,
...
- Each lane records agent evidence: actor status, actor log tail or last
  ...
  installed, initialized, configured, ran, or improved Humanish.
- Each lane gets a meaningful-use assessment:
  - `none`: setup did not reach usable Humanish proof;
  ...
  - `high-leverage`: the actor used Humanish evidence to find or articulate a
    concrete product or Humanish improvement.
- Agent feedback is classified as actionable, duplicate, vague, unsafe for
  public filing, or not captured.
- For Humanish-owned blockers, either implement a scoped fix with tests or file
...
- The full `humanish` repo.
- Ignored `.humanish/` runtime artifacts for verification.
...
- Humanish source and focused tests only when needed to unblock the study:
  - `src/oss-meta-lab.ts`
  - `src/run.ts`
  - `src/program.ts`
  - `src/labs.ts`
...
- GitHub issues in `danielgwilson/humanish` only after public-safety review.

Runtime/local-only:

- `.humanish/` run artifacts.
- `.humanish/local/labs/*.yaml` ignored private lab manifests.
- `.humanish/local/private-dogfood/**` ignored private review notes if raw
...
4. For each lane, fill the lane evaluation table in a receipt:
   ...
   - nested Humanish run/verify/Observer status;
   - setup-quality checks;
   - generated Humanish file presence and quality class;
   - actor evidence captured;
   - meaningful-use rating;
   - feedback classification;
   - blocker or next fix.
5. If evidence is insufficient because Humanish lacks a safe telemetry surface,
   implement the smallest public-safe telemetry improvement and rerun the
   relevant proof.
6. If a lane fails because of a Humanish-owned bootstrap issue, implement or
...
HUMANISH_OSS_META_ACTOR_TIMEOUT_MS=480000 \
HUMANISH_OSS_META_ACTOR_MODEL=gpt-5.4-mini \
HUMANISH_OSS_META_COMPLETION_TIMEOUT_MS=900000 \
...
- `.humanish/runs/<run-id>/run.json`
- `.humanish/runs/<run-id>/review.json`
- `.humanish/runs/<run-id>/review.md`
- `.humanish/runs/<run-id>/events.ndjson`
- `.humanish/runs/<run-id>/observer/index.html`
- `.humanish/runs/<run-id>/observer/observer-data.json`
- `.humanish/runs/<run-id>/setup-quality/*.json`
- `docs/goals/private-repo-agent-dogfood/receipts/*.md`
- Public-safe GitHub issues in `humanish` for Humanish-owned defects.
...
- Transcript-only success: actor says it used Humanish without filesystem,
...
- Workspace-root install failure: package-manager bootstrap fails before
  Humanish init, as tracked in issue #86.
...
- Prefer fixing small Humanish-owned harness defects immediately when they block
...
- E2B Desktop sandboxes for Humanish OSS meta-lab lanes.

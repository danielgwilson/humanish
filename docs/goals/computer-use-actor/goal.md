...
- **Registry entry**: `openai-computer-use` in `actorRegistry` — `runSession`-only, since the
  loop already returns a finished `humanish.actor-trace.v1`; capabilities reuse
...
- **Live (spend-gated)** — `tests/computer-use-actor.live.test.ts`: triple-gated
  (`HUMANISH_LIVE_CUA=1` + both API keys + lazy peer import). A real E2B desktop showing a local

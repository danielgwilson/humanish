...
Linux acceptance and the remaining Mac media execution gate. Published behavior is described
...
| Mobile and media | Hosted viewport/emulation, desktop geometry checks, bounded dwell and declared camera feed; candidate optional camera and speech passed a two-person Linux conference; hosted audio-only uses the same worker | Mac media execution remains unqualified. Hosted camera+speech is unsupported; English recognition and robotic voice have fidelity limits; raw AV recording, forced TURN, physical-device and touch fidelity are unproven |

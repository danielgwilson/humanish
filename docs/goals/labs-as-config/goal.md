...
Mimetic's thesis is "install once, configure a reusable user-study (lab) per project" — not
...
- **`mimetic.lab.v2`** (`src/lab-config.ts`): a lab is a composition — `subject` × `actors[]` ×
...
- The three built-ins are now example v2 configs (`mimetic/labs/*.yaml`); `mimetic init`
  scaffolds v2.
...
1. **Necessity** (`tests/lab-structural.test.ts`): `LabKind`, `isLabKind`, the
   `switch (resolved.manifest.kind)`, the three `run*LabCommand` fns, and the `mimetic.lab.v1`

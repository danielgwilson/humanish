...
The deterministic BrowserPersona driver that has always powered `humanish run --app-url` is now
...
- **New backend** `src/scripted-browser-lab.ts` for the composition `subject.source: app-url`
  ...
  resolves a committed scenario (`humanish/scenarios/<ref>.yaml` or a cwd-clamped path) whose
...
- `policies.redactScreenshots: true` is REJECTED (blur is not implemented here; a silently
  ...
  `.humanish/`; the bundle, verify warnings, and the lab warning all state the raw posture.
...
- `HUMANISH_SCRIPTED_LAB_BROWSER_MISSING` (no Chrome/Chromium found before a live run) has no
  ...
  `run --app-url`'s existing `HUMANISH_BROWSER_APP_CAPTURE_FAILED` resolution.
...
- Live (actuation-gated `HUMANISH_LIVE_SCRIPTED=1`, requires a local Chrome/Chromium; provider

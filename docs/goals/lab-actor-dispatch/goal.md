...
`mimetic.lab.v2` configs with `subject.source: app-url` now dispatch end-to-end:
...
- The lab fills the previously-declared-but-unfilled `RunStream.actor` seam with the loop's
  `mimetic.actor-trace.v1`, persists a full run bundle (`run.json` / `review.*` /
...
- Live (spend-gated, `MIMETIC_LIVE_CUA=1` + both keys): `tests/cua-actor-lab.live.test.ts`

...
export function Sidebar({ data, history, onRuns }: { data: ObserverData; history: HistoryIndex | null; onRuns: () => void }) {
  ...
  const allRuns: NonNullable<HistoryIndex>["runs"] = history?.runs.length ? history.runs : [{ runId: data.run.runId, status: data.run.status, createdAt: data.run.createdAt, href: "", mode: data.run.mode, streamCount: data.streams.length, ...(data.runtime ? { runtimeState: data.runtime.state } : {}) }];
  const matches = allRuns.filter((run) => (!onlyRunning || (run.runtimeState === "running" || (!run.runtimeState && (run.status === "running" || run.status === "preparing")))) && run.runId.toLowerCase().includes(query.toLowerCase()));
  return <div className="side"><nav aria-label="Run library"><div className="grp">
    ...
    {matches.slice(0, limit).map((run) => {
      ...
      const content = <><span className={`dot${(run.runtimeState === "running" || run.status === "running") ? " active" : ["pass", "passed", "complete"].includes(run.status) ? " ok" : " bad"}`} />

...
export function ParticipantCard({ stream, name, onOpen, liveThumb = false, pinned = false, compared = false, onPin, onCompare, now = Date.now() }: {
  ...
  const detailsLabel = `Participant details: ${name}${warnings.length ? ` (${warnings.length} notices)` : ""}`;
  ...
  return <article className={`panel card${pinned ? " pinned" : ""}`} data-stream-id={stream.id} aria-label={name}
    ...
    <div className="card-preview">
      <div className="thumb" style={{ aspectRatio: viewport ? `${viewport.width} / ${viewport.height}` : "16 / 10" }}>
        ...
        {previewLabel ? <span className="th-pill th-source">{previewLabel}</span> : null}
        {liveThumb && liveUrl ? <span className="th-connection">Read-only · connection unverified</span> : null}
      </div>
    </div>
    <div className="card-actions">
      {onPin ? <button type="button" className="card-icon" title={pinned ? "Unpin participant" : "Pin participant"} aria-pressed={pinned} aria-label={`${pinned ? "Unpin" : "Pin"} participant ${name}`} onClick={() => onPin(stream.id)}><ReviewIcon name="pin" /></button> : null}
      {onCompare ? <button type="button" className="card-icon" title={compared ? "Remove from comparison" : "Add to comparison"} aria-pressed={compared} aria-label={`${compared ? "Remove from" : "Add to"} comparison: ${name}`} onClick={() => onCompare(stream.id)}><ReviewIcon name="compare" /></button> : null}
      <Popover triggerClassName={`card-icon${warnings.length || flagged ? " has-notices" : ""}`} label={detailsLabel}
        trigger={<ReviewIcon name={warnings.length || flagged ? "notice" : "more"} />}>
        <div className="card-details">
          <h2>{name}</h2>
          <dl><dt>Participant</dt><dd>{label}</dd><dt>Persona</dt><dd>{stream.sim.personaId}</dd>
            <dt>Status</dt><dd>{statusLabel}</dd><dt>Preview</dt><dd>{previewLabel ?? "Recorded"}</dd>
          ...
          {warnings.length ? <section className="card-warnings" aria-label="Participant notices"><h3>{warnings.length} {warnings.length === 1 ? "notice" : "notices"}</h3><ul>{warnings.map((event) => <li key={event.id}>{event.message}</li>)}</ul></section> : null}
          <button type="button" className="review-tool" onClick={() => onOpen(stream.id)}>Open participant</button>
        </div>
      </Popover>
    </div>
    <div className="card-caption">
      <button type="button" className="card-name" title={name} onClick={() => onOpen(stream.id)}>{name}</button>
      <span className={`card-outcome${active ? " active" : ""}${flagged ? " flagged" : ""}`} title={outcome}>{outcome}</span>
    </div>

...
export function buildLiveTerminalProductBundle(args: {
  ...
  return {
    ...
    redaction: {
      status: "passed",
      notes: `Live terminal-product run: the in-sandbox agent's output was captured via commands.run onStdout/onStderr and scrubbed (literal known values incl. the runtime key) THEN redacted (shape patterns) AT THE SOURCE before persisting. ${args.runtimeAuth === "openai-egress" ? `Runtime auth openai-egress: the raw key from ${args.runtimeAuthKeyName} was supplied only to E2B's external api.openai.com HTTPS header transform; Codex received an inert CODEX_API_KEY placeholder. Every sandbox process retained a spendable OpenAI proxy capability until teardown; additional provider calls may not appear in the Codex usage ledger.` : `Runtime auth openai-env: the runtime key (${args.runtimeAuthKeyName}) was injected ONLY into the command-scoped codex invocation, never sandbox-global env or metadata; only its NAME appears in evidence.`} Subject provenance is UNPINNED (public-surface study).`
    },
    artifacts: {
      run: "run.json",
      reviewJson: "review.json",

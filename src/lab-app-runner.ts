export const LAB_APP_RUNNER_PLAN_SCHEMA = "mimetic.lab-app-runner-plan.v1";
...
export function renderLabAppRunnerShell(args: {
  ...
  return {
    ...
    script: [
      ...
      "APP_RUNNER_LOG=\"${APP_RUNNER_LOG:-mimetic-app-runner.log}\"",
      "",
      renderWaitFunction(),
      "",
      renderOpenFunction(),
...
function renderOpenFunction(): string {
  ...
  if command -v google-chrome >/dev/null 2>&1; then
    nohup google-chrome --no-first-run --no-default-browser-check --disable-default-apps --user-data-dir=".mimetic-lab-\${profile}-chrome" --window-size="\${width},\${height}" "$url" >/dev/null 2>&1 &
